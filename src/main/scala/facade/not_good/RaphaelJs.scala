...
//    // conflicts with above def getFont(family: String, weight: Double = ???, style: String = ???, stretch: String = ???): RaphaelFont = js.native
//    var height: Double = js.native
...
//// conflicts with above    def snapTo(values: js.Array[Double], value: Double, tolerance: Double = ???): Double = js.native
